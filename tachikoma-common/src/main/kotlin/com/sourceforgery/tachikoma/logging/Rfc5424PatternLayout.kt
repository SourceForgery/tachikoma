...
class Rfc5424PatternLayout
    private constructor(
        ...
        init {
            ...
            mdcExcludes =
                toTrimmedArray(excludes)
                    ?.also {
                        c = ExcludeChecker()
                    }
            mdcIncludes =
                toTrimmedArray(includes)
                    ?.also {
                        c = IncludeChecker()
                    }
        ...
        private fun appendMessage(
            ...
            val text =
                if (message is StructuredDataMessage || message is MessageCollectionMessage<*>) {
                    message.format
                } else {
        ...
        private fun appendStructuredElements(
            buffer: StringBuilder,
            event: LogEvent,
        ) {
            val message = event.message
        ...
        private fun addStructuredData(
            sdElements: MutableMap<String, StructuredDataElement>,
            data: StructuredDataMessage,
        ) {
            val map = data.data
            val id = data.id
            val sdId = getId(id)
        ...
        private fun escapeNewlines(
            ...
            return if (null == replacement) {
                ...
                NEWLINE_PATTERN.replace(text, replacement)
            }
        }

        private fun escapeException(
            event: LogEvent,
            text: String,
            buffer: StringBuilder,
        ) {
